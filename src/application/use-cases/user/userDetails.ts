...
export function removePasswordField(object: any) {
  let { _id, username, email, phone, image, createdAt, isBlocked, updatedAt } =
    object;
...
export const getSerachLocationData = async (
  data: string,
  userRepository: ReturnType<UserRepositoryMongoDBType>
) => {
  const location = await userRepository.searchLocationValue(data);
...
export const allManagerGet = async (
  repository: ReturnType<UserRepositoryMongoDBType>
) => {
  const data = await repository.getAllManager();

  return data;
};

export const allDataGet = async (
  repository: ReturnType<UserRepositoryMongoDBType>
) => {
  try {
...
export const allRoleDataGet = async (
  id: string,
  repository: ReturnType<UserRepositoryMongoDBType>
) => {
  try {
    const data = await repository.getAllRoleData(id);
    return data;
  } catch (err) {
    throw new AppError("Something Went Wrong", HttpStatus.NOT_ACCEPTABLE);
  }
